...
//Modal for character search//

$("#modal-search-btn").on("click", function (event) {
    event.preventDefault();
  
    var SEARCH = $("#search-input").val();
    const KEY = '10161297457820113';
    var queryURL = `https://cors-anywhere.herokuapp.com/http://superheroapi.com/api.php/${KEY}/search/${SEARCH}`;
    ...
    $.ajax({
      ...
      var stats = {
        ...
        strength: response.results[0].powerstats.strength,
        speed: response.results[0].powerstats.speed,
        durability: response.results[0].powerstats.durability,
        power: response.results[0].powerstats.power,
        combat: response.results[0].powerstats.combat
      }
  
      var heroImage = $("<img>").attr("src", response.results[0].image.url).addClass("charPic");
      $("#photo-view").append(heroImage);
  
      $("#charInfo").text(JSON.stringify("Intelligence: " + stats.intelligence));
      $("#charInfo2").text(JSON.stringify("Strength: " + stats.strength));
      $("#charInfo3").text(JSON.stringify("Speed: " + stats.speed));
      $("#charInfo4").text(JSON.stringify("Durability: " + stats.durability));
      $("#charInfo5").text(JSON.stringify("Power: " + stats.power));
      $("#charInfo6").text(JSON.stringify("Combat: " + stats.combat));
